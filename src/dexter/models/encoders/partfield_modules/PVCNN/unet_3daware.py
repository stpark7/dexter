...
def conv1x1triplane3daware(in_channels, out_channels, order="xz", **kwargs):
    return ConvTriplane3dAware(
        lambda inp, out: conv1x1(inp, out, **kwargs), in_channels, out_channels, order=order
    )
...
class UpConv3dAware(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, merge_mode="concat", with_conv=False
    ):  # up_mode='transpose', ):
        ...
        if self.merge_mode == "concat":
            ...
            self.block = ResnetBlock3dAware(
                in_channels=in_channels + out_channels, out_channels=out_channels
            )
        else:
            self.norm1 = Normalize(in_channels)
            self.block = ResnetBlock3dAware(in_channels=in_channels, out_channels=out_channels)
