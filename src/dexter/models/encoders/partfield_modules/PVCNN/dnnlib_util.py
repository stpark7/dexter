...
class TimingsMonitor:
    ...
    def __init__(
        self, device, enabled=True, timing_names: list[str] = [], cuda_timing_names: list[str] = []
    ):
        """
        Usage:
...
def download_from_s3(file_path, target_path, cfg):
    ...
    logger.info(
        f"finish download from ! s3://{bucket_name}/{file_key} to {target_path} %.1f sec"
        % (time.time() - tic)
    )
...
def write_ckpt_to_s3(cfg, all_model_dict, ckpt_name):
    ...
    upload_thread = threading.Thread(
        target=upload_to_s3, args=(buffer, bucket_name, key, config_dict)
    )
    upload_thread.start()
...
def readable_size(num_bytes: int) -> str:
    return calmsize(
        num_bytes
    )  ## '' if math.isnan(num_bytes) else '{:.1f}'.format(calmsize(num_bytes))
