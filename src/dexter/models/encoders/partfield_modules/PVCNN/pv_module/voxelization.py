...
def my_voxelization(features, coords, resolution):
    ...
    result = torch.zeros(
        b, c + 1, resolution * resolution * resolution, device=features.device, dtype=features.dtype
    )
    r = resolution
    ...
    # Use scatter_add instead of scatter_ with reduce="add" for torch.compile compatibility
    out_feature = result.scatter_add(dim=2, index=indices.long(), src=features)

    cnt = out_feature[:, -1:, :]
    zero_mask = (cnt == 0).float()
    cnt = cnt * (1 - zero_mask) + zero_mask * 1e-5
    vox_feature = out_feature[:, :-1, :] / cnt
    return vox_feature.view(b, c, resolution, resolution, resolution).to(features.dtype)
...
class Voxelization(nn.Module):
        ...
        with torch.no_grad():
            ...
            if self.normalize:
                norm_coords = (
                    norm_coords
                    / (
                        norm_coords.norm(dim=1, keepdim=True).max(dim=2, keepdim=True).values * 2.0
                        + self.eps
                    )
                    + 0.5
                )
            elif self.scale_pvcnn:
    ...
    def extra_repr(self):
        return "resolution={}{}".format(
            self.r, f", normalized eps = {self.eps}" if self.normalize else ""
        )
