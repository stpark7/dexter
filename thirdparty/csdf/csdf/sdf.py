...
def index_vertices_by_faces(vertices_features, faces):
    ...
    indices = faces[..., None].expand(-1, -1, vertices_features.shape[-1])
...
class _UnbatchedTriangleDistanceCuda(torch.autograd.Function):
    ...
    def forward(ctx, points, face_vertices):
        ...
        normal = torch.zeros((num_points, 3), device=points.device, dtype=points.dtype)
        dist_sign = torch.zeros((num_points), device=points.device, dtype=torch.int32)
        ...
        ctx.save_for_backward(
            points.contiguous(), face_vertices.contiguous(), min_dist_idx, dist_type
        )
    ...
    def backward(ctx, grad_dist, grad_normal, grad_dist_sign, grad_face_idx, grad_dist_type):
        ...
        _C.unbatched_triangle_distance_backward_cuda(
            grad_dist, points, face_vertices, face_idx, dist_type, grad_points
        )
