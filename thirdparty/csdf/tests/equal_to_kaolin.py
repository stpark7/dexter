...
for mesh in os.listdir("meshes"):
    test_mesh = trimesh.load(os.path.join("meshes", mesh), force="mesh", process=False)
    ...
    signs = torch.where(signs_, -torch.ones_like(signs_).int(), torch.ones_like(signs_).int())
    g = torch.autograd.grad([dis.sum()], [x], create_graph=True, retain_graph=True)[0]

    dis_, normal, dis_sign, dis_faces, dis_types = csdf.compute_sdf(x, face_verts_)
    if mesh != "suzanne.obj":
        if not torch.allclose(signs, dis_sign):
            print("wrong in sign")
            exit(0)
    g__ = 2 * dis_.unsqueeze(1).sqrt() * normal
    g_ = torch.autograd.grad([dis_.sum()], [x], create_graph=True, retain_graph=True)[0]
