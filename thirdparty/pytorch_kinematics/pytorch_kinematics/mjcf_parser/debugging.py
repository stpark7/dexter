...
def _get_actual_current_stack_trace():
    ...
    for raw_stack_item in raw_stack:
        ...
        if stack_item.filename.startswith(MODULE_PATH) and not stack_item.filename.endswith(
            "_test.py"
        ):
            break
        else:
        ...
        if debug_mode():
            ...
            if xml_line:
                message_lines.extend(
                    [stripped_xml_line, self._generate_debug_message_from_xml_line(xml_line)]
                )
        else:
            message_lines.extend(
                [
            ...
            with open(os.path.join(dump_dir, str(elem_id) + ".dump"), "w") as f:
                ...
                for attrib_name, stack in six.iteritems(debug_info.attribute_stacks):
                    ...
                    if stack[-1] == debug_info.init_stack[-1]:
                        ...
                        if attrib_value is not None:
                            dump_stack(
                                'Attribute {}="{}"'.format(attrib_name, attrib_value), stack, f
                            )
                        else:
