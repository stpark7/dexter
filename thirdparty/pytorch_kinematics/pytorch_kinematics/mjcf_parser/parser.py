...
def from_xml_string(
    xml_string, escape_separators=False, model_dir="", resolve_references=True, assets=None
):
...
def from_file(
    file_handle, escape_separators=False, model_dir="", resolve_references=True, assets=None
):
            ...
            if escape_separators:
                ...
                for name, value in six.iteritems(xml_child.attrib):
                    new_value = value.replace(
                        constants.PREFIX_SEPARATOR_ESCAPE, constants.PREFIX_SEPARATOR_ESCAPE * 2
                    )
