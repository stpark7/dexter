...
from . import base, constants, debugging, util
    ...
    def clear(self):
        if self._required:
            raise AttributeError(
                "Attribute {!r} of element <{}> is required".format(self._name, self._parent.tag)
            )
        else:
            self._force_clear()
...
class Keyword(_Attribute):
    ...
    def __init__(
        self, name, required, parent, value, conflict_allowed, conflict_behavior, valid_values
    ):
...
class Array(_Attribute):
    ...
    def _check_shape(self, array):
        ...
        if self._length and actual_length > self._length:
            raise ValueError(
                "Expect array with no more than {} entries: got {}".format(self._length, array)
            )
        return array
...
class BasePath(_Attribute):
    ...
    def __init__(
        self, name, required, parent, value, conflict_allowed, conflict_behavior, path_namespace
    ):
...
class File(_Attribute):
    ...
    def get_contents(self):
        ...
        if self._value is None:
            raise RuntimeError(
                "You must assign a value to this attribute before querying the contents."
            )
