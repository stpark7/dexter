...
class Joint(object):
    ...
    def __repr__(self):
        return "Joint(name='{0}', offset={1}, joint_type='{2}', axis={3}, range={4})".format(
            self.name, self.offset, self.joint_type, self.axis, self.range
        )
...
class Frame(object):
    ...
    def get_transform(self, theta):
        ...
        if self.joint.joint_type == "revolute":
            t = tf.Transform3d(
                rot=tf.axis_angle_to_quaternion(theta * self.joint.axis), dtype=dtype, device=d
            )
